...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Get all the Factors with the given evidence
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        # Eliminate Variable in the given order
        for var in eliminationOrder:
            # First join all factors that contains the variable to eliminate
            factors, newFactor = joinFactorsByVariable(factors, var)
            # If only one unconditioned variable in the new factor then discard
            if len(newFactor.unconditionedVariables()) > 1:
                f_factor = eliminate(newFactor, var)
                factors.append(f_factor)
        # Join all factors and normalize
        newFactor = normalize(joinFactors(factors))
        return newFactor
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        # If it sums to 0 then end function 
        if total == 0:
            return
        
        # Update probability for each item
    ...
    def sample(self):
        ...
        # Iterate through list of probabiliy while cumulating probability
        # until the value of the sample is less than th cum sum
        i = 0
        cumSum = probs[0]
        while sample > cumSum:
            i += 1
